...
class RecordManager {
    ...
    let file_path = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending("/"+SmallTools.dateConvertString(date: Date())+".wav")
    ...
    func play(filePath:URL) {
        do {
            player = try AVAudioPlayer(contentsOf:
                filePath)//URL(fileURLWithPath: file_path!))
