...
class AudioTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let url1 = URL(fileURLWithPath:path as! String)
        let recorder = RecordManager()
        recorder.play(filePath: url1)
        print("播放完毕")
        
    }
    
    func refreshData(){
        let f = NSFetchRequest<NSFetchRequestResult>(entityName: "AudioNote")
        do {
            try
